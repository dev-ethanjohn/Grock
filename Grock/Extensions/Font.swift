...
// View Extensions - for general Views
extension View {
...
// Text Extensions - specifically for Text concatenation with +
extension Text {
    ...
    func fuzzyBubblesFont(_ size: CGFloat, weight: Font.Weight = .regular) -> Text {
        let fontName = weight == .bold || weight == .semibold || weight == .heavy || weight == .black ?
            "FuzzyBubbles-Bold" : "FuzzyBubbles-Regular"
        return self.font(.custom(fontName, size: size))
    }
}
