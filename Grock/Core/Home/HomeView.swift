...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
                 ...
                 // Use ONE unified fullScreenCover for ALL cart presentations
                 .fullScreenCover(item: $viewModel.selectedCart) { cart in
                     CartDetailScreen(cart: cart)
                         .onDisappear {
                             // Optional: Any cleanup when cart detail is dismissed
                         }
                 }
                 .onAppear {
                     viewModel.loadCarts()
            ...
            VStack(alignment: .leading, spacing: 0) {
                if viewModel.carts.isEmpty {
                    ...
                    ScrollView {
                        ...
                        LazyVStack(spacing: 12) {
                            ForEach(viewModel.displayedCarts) { cart in
                                // CHANGED: Use sheet presentation instead of NavigationLink
    ...
    // NEW: Replace NavigationLink with a button that triggers fullScreenCover
