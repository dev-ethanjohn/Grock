...
struct BrowseVaultItemRow: View {
    ...
    private var minusButton: some View {
        ...
        .buttonStyle(.plain)
        .contentShape(Rectangle())
        .disabled(currentQuantity <= 0 || isFocused)  // <-- Add isFocused to disabled
        .opacity((currentQuantity <= 0 || isFocused) ? 0.5 : 1)  // <-- Fade when focused
    }
    ...
    private var plusButton: some View {
        ...
        .overlay(
            Circle()
                .stroke(plusButtonStrokeColor, lineWidth: 1)
                .opacity(isFocused ? 0.3 : 1)  // <-- Fade stroke when focused
        )
        .contentShape(Circle())
        .buttonStyle(.plain)
        .disabled(isFocused)  // <-- Disable when focused
        .opacity(isFocused ? 0.5 : 1)  // <-- Fade when focused
    }
    ...
    var body: some View {
        HStack(alignment: .bottom, spacing: 4) {
            ...
            .animation(.spring(response: 0.35, dampingFraction: 0.7), value: currentQuantity)
            .animation(.spring(response: 0.3, dampingFraction: 0.7), value: isFocused)  // <-- Add animation for focus state
            .padding(.top, 6)
        }
        .padding(.bottom, 4)
        .padding(.horizontal)
    ...
    private func handlePlus() {
        ...
        if storeItem.isShoppingOnlyItem {
            ...
            // Vault item
            if findCartItem() != nil {  // <-- Fixed: Check if cart item exists
